 ...
 * moder
 */
#define GPIOX_MODER_(a,b,c)   GPIO ## a->MODER = (GPIO ## a->MODER & ~(3 << (2 * b))) | (c << (2 * b));
#define GPIOX_MODER(a, b)     GPIOX_MODER_(a, b)
/**
 * otyper
 */
#define GPIOX_OTYPER_(a, b, n)  GPIO ## a->OTYPER = (GPIO ## a->OTYPER & ~ ((1<<b))) | n<<b;

#define GPIOX_OTYPER(a, b)		GPIOX_OTYPER_(a, b)
/**
 * ospeedr
 */
#define GPIOX_OSPEEDR_(a, b, n)		GPIO ## a->OSPEEDR = (GPIO ## a->OSPEEDR & ~ ((0x3)<<(2 * b))) | n<<(2 * b);

#define GPIOX_OSPEEDR(a, b)			GPIOX_OSPEEDR_(a, b)
/**
 * pupdr
 */
#define GPIOX_PUPDR_(a, b, n)		GPIO ## a->PUPDR =(GPIO ## a->PUPDR & ~ ((0x3)<<(2 * b))) | n<<(2 * b);
